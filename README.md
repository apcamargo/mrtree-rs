...
Build and install `mrtree-rs` from this repository with Cargo:

```sh
cargo install --path .
```

Alternatively, build the binary without installing it:

```sh
cargo build --release
```

This project targets Rust `1.91` or newer.
...
- Input must be a TSV file with one sample ID column and at least two clustering columns.
- Clustering labels must be non-negative integers.
